...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        total_loss = classifier_loss + im_loss + consistency_loss
        wandb.log({"total loss":total_loss.item(),"cls loss":classifier_loss.item(), "im_loss":im_loss.item(),"consistency loss":consistency_loss.item()})
            

        #classifier_loss = L2(outputs_stg,outputs_test)
        optimizer.zero_grad()
        total_loss.backward()
        print(f'Task: {args.name}, Iter:{iter_num}/{max_iter} \t total_loss {total_loss.item():.4f}')
        optimizer.step()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--rlcc', type=int, default=0)
    parser.add_argument('--soft_pl', type=int, default=0)
